...
def get_config(description, model_name, phase,
    ...
    parser.add_argument("--run_name", type=str, default="ct2xray_real_gan_meta")
    if cmd_opts: arg = parser.parse_args(cmd_opts)
    else: arg = parser.parse_args()
    # arg.run_name = 'v3'
    config_file = path.join(config_dir, f"{model_name}.yaml")  # config/xraysyn_test.yaml

    # Load default and run configs
    default = load_config(config_file)

    # Update default config by run config
    run_file = path.join(run_dir, f"{model_name}.yaml")  # run/xraysyn_test.yaml
    ...
    run_dir = path.join(run_dir, model_name, arg.run_name)  # run/xraysyn_test/ct2xray_real_gan_meta
    if not path.isdir(run_dir): os.makedirs(run_dir)

    # Save and return config
    save_config(default, path.join(run_dir, f"{phase}_opts.yaml"))  # run/xraysyn_test/ct2xray_real_gan_meta/train.yaml

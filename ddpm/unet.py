...
if __name__ == '__main__':
    model = UNet(in_ch=8)
    input = torch.randn((10, 8, 32, 32, 32))
    x_ray = torch.randn((10, 1, 128, 128))
    cond = torch.randn((1, 3, 128, 128))
    x_encoder1 = x_encoder(n_class=1)
    x_ray_cond = x_encoder1(x_ray)
    time = torch.randn((10,))
    out_hor = model(x_t=input, t=time, self_cond=x_ray_cond)
    print(out_hor.shape)

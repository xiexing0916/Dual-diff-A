...
class XraySynModel(Base):
    ...
    def ct2xray(self, vol, bone,T_in):  # CTvol, bone mask, 视角矩阵T_in
        vol = 0.0008088*(vol*5000-1000)+1.030
        bone_vol = vol * bone  # 公式4
        tissue_vol = vol * (1-bone)  # 公式4
        bone_proj = self.proj(bone_vol, T_in)  # forward,生成t_bone 公式4
        tissue_proj = self.proj(tissue_vol, T_in)  # forward,生成t_tissue 公式4
        atten_bone = self.bone_absorb(bone_proj)  # forward,生成I_bone 公式6
        atten_tissue = self.tissue_absorb(tissue_proj) # forward,生成I_tissue 公式6
        atten_proj = atten_bone+atten_tissue # forward,相加生成I_atten指数上的值I 公式6
        out_new = torch.exp(atten_proj).sum(dim=1).view(vol.shape[0],1,128,128) # 对I求指数，得到I_atten 公式6
        out_new = self.norm(out_new.max() - out_new) # 公式5
    ...
    def test(self, xray):
        ...
        with torch.no_grad():
            ...
            print("vol_in:", vol_in.shape)
            vol_in1 = torch.squeeze(vol_in).to('cpu')
            vol_in1 = (vol_in1 - torch.min(vol_in1)) / (torch.max(vol_in1) - torch.min(vol_in1))
            tensor2img = transforms.ToPILImage()
            img2tensor = transforms.ToTensor()
            vol_in2 = tensor2img(vol_in1[:, 70, :])
            vol_in2.show()  # V_BP
            vol_pred_temp = self.net3d(vol_in)*0.5+0.5 # V_CT和V_mask
            print("vol_pred_temp", vol_pred_temp.shape)
            bone_mask = vol_pred_temp[:,[0]]
            print("bone_mask", bone_mask.shape)
            # bone_mask1 = torch.squeeze(bone_mask).to('cpu')
            # bone_mask2 = tensor2img(bone_mask1[:,70,:])
            # bone_mask2.show()  # V_mask
            bone_ct = vol_pred_temp[:,[1]]*bone_mask
            bone_ct1 = torch.squeeze(bone_ct).to('cpu')
            bone_ct2 = tensor2img(bone_ct1[:,70,:])
            # bone_ct2.show()  # V_CT_bone
            tissue_ct = vol_pred_temp[:,[2]]*(1-bone_mask)
            tissue_ct1 = torch.squeeze(tissue_ct).to('cpu')
            tissue_ct2 = tensor2img(tissue_ct1[:,70,:])
            # tissue_ct2.show()  # V_CT_tissue
            vol_pred = bone_ct + tissue_ct
            vol_pred1 = torch.squeeze(vol_pred).to('cpu')
            vol_pred2 = tensor2img(vol_pred1[:,70,:])
            # vol_pred2.show()   # V_CT
            vol_pred3 = img2tensor(vol_pred2)
            xray_other_pred = []
            mat_other_pred = []
            xray_other_refine = []
            mat_other_refine = []
            for view in self.views:
                ...
                xray2_pred, mat2_pred = self.ct2xray(vol_pred, bone_mask, T_other) # I_CT
                xray2_pred1 = torch.squeeze(xray2_pred).to('cpu')
                xray2_pred2 = tensor2img(xray2_pred1)
                # xray2_pred2.show()
